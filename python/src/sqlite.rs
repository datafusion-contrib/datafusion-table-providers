...
use datafusion_table_providers::{sql::db_connection_pool::{sqlitepool::{SqliteConnectionPool, SqliteConnectionPoolFactory}, DbConnectionPool}, sqlite::SqliteTableFactory};
use pyo3::prelude::*;

use crate::{utils::{to_pyerr, wait_for_future}, RawTableProvider};
...
impl RawSqliteTableFactory {
    ...
    pub fn new(py: Python, path: &str, mode: String, busy_timeout_s: f64, attach_databases: Option<Vec<String>>) -> PyResult<Self> {
        let mode = mode.as_str().into();
        let busy_timeout = Duration::from_secs_f64(busy_timeout_s);
        let attach_databases = attach_databases.map(|d| d.into_iter().map(|s| Arc::from(s)).collect());
        let factory = SqliteConnectionPoolFactory::new(path, mode, busy_timeout).with_databases(attach_databases);
    ...
    pub fn tables(&self, py: Python) -> PyResult<Vec<String>> {
        wait_for_future(py, async {
            ...
            let conn_async = conn.as_async().ok_or(to_pyerr("Unable to create connection to sqlite db".to_string()))?;
    ...
    pub fn get_table(&self, py: Python, table_reference: &str) -> PyResult<RawTableProvider> {
        let table = wait_for_future(py, self.factory.table_provider(table_reference.into())).map_err(to_pyerr)?;
