...
pub enum Error {
    ...
    #[snafu(display("Unexpected reference count on DuckDB table.\nThis is an internal error. Submit a bug: https://github.com/datafusion-contrib/datafusion-table-providers"))]
    UnexpectedReferenceCountOnDuckDBTable,
}
...
impl TableProviderFactory for DuckDBTableProviderFactory {
    ...
    async fn create(
        ...
        let (duckdb, constraints_and_indexes) =
            TableCreator::new(name.clone(), Arc::clone(&schema), Arc::new(pool))
        ...
        if let Some(temp_directory) = options.get("temp_directory") {
