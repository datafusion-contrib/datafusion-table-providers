...
impl DataSink for DuckDBPartitionedDataSink {
    ...
    async fn write_all(
        ...
        let (batch_tx, batch_rx): (Sender<(String, PathBuf)>, Receiver<(String, PathBuf)>) = mpsc::channel(20);
        ...
        const ROWS_PER_PARTITION_BUFFER: usize = 50 * 5 * 122_880; // 750 MiB
        ...
        while let Some(batch) = data.next().await {
            ...
            let batches = partitioner.partition_batch(&batch).unwrap();
...
fn insert_overwrite(
    pool: Arc<DuckDbConnectionPool>,
    table_definition: &Arc<TableDefinition>,
    batch_rx: Receiver<(String, PathBuf)>,
    on_conflict: Option<&OnConflict>,
    ...
    let num_rows = write_parquet_files_to_partitioned_tables(&table_definition, &tx, schema, batch_rx, on_conflict, cloned_pool2)
        .map_err(to_retriable_data_write_error)?;
...
impl PartSink {
    ...
    fn roll(&mut self, schema: &SchemaRef) -> datafusion::common::Result<()> {
        ...
        // Disable compression and use an older, simpler writer version to avoid
        // CPU / I/O overhead from compression, dictionaries, and newer encodings.
        // Use a very large row group size to minimize per-file/row-group bookkeeping.
        let props = WriterProperties::builder()
            .set_writer_version(WriterVersion::PARQUET_1_0)
            //.set_writer_version(WriterVersion::PARQUET_2_0)
            .set_compression(datafusion::parquet::basic::Compression::SNAPPY)
            //.set_compression(datafusion::parquet::basic::Compression::ZSTD(ZstdLevel::default())) // Min Compression Level
            .set_max_row_group_size(1024 * 1024) // large row groups to reduce overhead
            .set_write_batch_size(1024 * 1024) // large write batches to reduce overhead
            .build();
