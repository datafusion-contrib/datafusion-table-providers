...
impl PartitionBuffer {
    ...
    fn get_next_worker(&self) -> &Sender<WorkerMessage> {
        let worker_idx = self.current_worker.fetch_add(1, std::sync::atomic::Ordering::Relaxed) % self.worker_senders.len();
    ...
    async fn flush_partition(&mut self, partition_id: &str, table_name: String) -> datafusion::common::Result<()> {
        if let Some(partition_batches) = self.buffers.remove(partition_id) {
            if !partition_batches.is_empty() {
                let worker_sender = self.get_next_worker();
...
/// Spawn pool of appender workers that handle batch writing concurrently
fn spawn_appender_workers(
    pool: Arc<DuckDbConnectionPool>,
    worker_count: usize,
    schema: SchemaRef,
) -> (Vec<Sender<WorkerMessage>>, Vec<JoinHandle<datafusion::common::Result<u64>>>) {
    let mut senders = Vec::new();
    let mut handles = Vec::new();

    // Shared locks per partition - much more efficient than global lock
    let partition_locks = Arc::new(Mutex::new(HashMap::<String, StdArc<Mutex<()>>>::new()));
    // Shared state to track which partitions have been created
    let created_partitions = Arc::new(Mutex::new(HashMap::<String, ()>::new()));

    for worker_id in 0..worker_count {
        let (sender, receiver) = mpsc::channel::<WorkerMessage>(2);
        let pool_clone = Arc::clone(&pool);
        let schema_clone = Arc::clone(&schema);
        let partition_locks_clone = Arc::clone(&partition_locks);
        let created_partitions_clone = Arc::clone(&created_partitions);
        ...
        let handle = tokio::task::spawn_blocking(move || {
            appender_worker(
                ...
                receiver, 
                schema_clone,
                partition_locks_clone,
                created_partitions_clone
            ).inspect_err(|err| {
                println!("Appender worker {} failed: {}", worker_id, err)
            })
        });
...
impl DataSink for DuckDBPartitionedDataSink {
    ...
    async fn write_all(
        ...
        // Spawn appender worker pool
        const WORKER_COUNT: usize = 4;
        let (worker_senders, worker_handles) = spawn_appender_workers(
            Arc::clone(&pool),             WORKER_COUNT, 
            Arc::clone(&schema)
        );

        // Buffer 128k rows per partition before sending to workers
        const ROWS_PER_PARTITION_BUFFER: usize = 5 * 122_880;
        ...
        while let Some(batch) = data.next().await {
            ...
            let millis = SystemTime::now()
                ...
                .map(|d| d.as_millis())
                .unwrap_or(0);
            let partition_id = (millis as usize) % 20;
        ...
        // Drop partition buffer to close worker channels
        ...
        for (worker_id, handle) in worker_handles.into_iter().enumerate() {
            match handle.await {
                Ok(Ok(rows)) => {
                    total_rows += rows;
                    tracing::debug!("Worker {} completed with {} rows", worker_id, rows);
                }
                Ok(Err(e)) => {
                    return Err(DataFusionError::Execution(format!(
                        "Worker {} failed: {}", worker_id, e
                    )));
                }
                Err(e) => {
                    return Err(DataFusionError::Execution(format!(
                        "Worker {} join error: {}", worker_id, e
                    )));
                }
            }
        }

        tracing::info!("All {} workers completed, total rows processed: {}", WORKER_COUNT, total_rows);
