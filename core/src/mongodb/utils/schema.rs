...
fn unify_types(type1: &DataType, type2: &DataType) -> DataType {
    match (type1, type2) {
        ...
        (DataType::Int32, DataType::Float64) | (DataType::Float64, DataType::Int32) => DataType::Float64,
...
mod tests {
    ...
    fn test_single_document_simple_types() {
        ...
        let field_map: HashMap<String, &DataType> = schema.fields()
            .iter()
    ...
    fn test_missing_fields_across_documents() {
        ...
        let field_names: std::collections::HashSet<&str> = schema.fields()
            .iter()
            .map(|f| f.name().as_str())
            .collect();
