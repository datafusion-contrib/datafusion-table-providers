...
async fn test_postgres_jsonb_list_struct_with_projected_schema(port: usize) {
    ...
    let df = ctx
        .sql("SELECT id, data FROM jsonb_list_struct_values ORDER BY id")
        .await
        .expect("DataFrame should be created from query");
...
async fn test_postgres_json_list_struct_with_projected_schema(port: usize) {
    let create_table_stmt = "
    CREATE TABLE json_list_struct_values (
        id INT PRIMARY KEY,
        data JSON
    );";

    let insert_table_stmt = r#"
    INSERT INTO json_list_struct_values (id, data) VALUES
        (1, '[{"id":"u1","email":"one@example.com"},{"id":"u2","email":"two@example.com"}]'),
        (2, '[]'),
        (3, null);
    "#;
    ...
    let table = SqlTable::new_with_schema(
        ...
        &sqltable_pool,
        Arc::clone(&projected_schema),
        "json_list_struct_values",
    );
    ctx.register_table("json_list_struct_values", Arc::new(table))
        .expect("Table should be registered");

    let df = ctx
        .sql("SELECT id, data FROM json_list_struct_values ORDER BY id")
        .await
        .expect("DataFrame should be created from query");
